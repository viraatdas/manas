...
- **Version source of truth:** `VERSION`/`BUILD` in `scripts/make-app.sh` (mac) and `MARKETING_VERSION`/`CURRENT_PROJECT_VERSION` in `ios/project.yml` (iOS). Bump both together.
- **Keep the two platforms on the same major.minor.** The Mac and iOS marketing versions must agree on `x.y`; only the patch is allowed to drift, because the two ship on different cadences and either side may need a fix the other does not. So mac `0.4.12` pairs with iOS `0.4.x`, and a feature that lands on both bumps the minor on both. The point is that "which iPhone build goes with this Mac build" is answerable by reading the numbers, instead of cross-referencing dates in App Store Connect.
  - When they are out of step, raise the trailing one to match rather than lowering the leading one — App Store Connect refuses any upload whose `CFBundleShortVersionString` is not higher than the last approved version, so iOS versions can only ever go up.
  - NOTE (2026-08-19): they are NOT in step yet. Mac is on `0.4.12` and iOS is on `1.0.1`, because iOS went to the App Store as `1.0` before this rule existed. Closing the gap means taking the Mac to `1.0.x` at the next release that warrants it; iOS cannot come down to `0.4.x`.
- **Release type on App Store Connect:** create every `appStoreVersion` with `releaseType: "AFTER_APPROVAL"`. `MANUAL` parks an approved build at Pending Developer Release until somebody presses a button, and it cannot be changed after approval (409). That is how 1.0 sat approved and invisible for three weeks.
